...
<body>
  ...
  <script>
    // ─── SAFE LOCALSTORAGE WRAPPER (Tomcat / file:// safe) ───
    var Store = {
      get: function(key) {
    ...
    var toggleLbl  = document.getElementById('toggleLabel');
    var htmlEl     = document.documentElement;

    if (Store.get('theme') === 'light') {
      htmlEl.classList.add('light');
      toggleIcon.textContent = '☀️';
      toggleLbl.textContent  = 'Light';
    }

    toggleBtn.addEventListener('click', function() {
      var isLight = htmlEl.classList.toggle('light');
    ...
    var checklistVisible = false;

    // Load saved checklist data
    try {
      var savedChk = Store.get('wb_checklists');
      if (savedChk) { checklistData = JSON.parse(savedChk); }
    ...
    function openWhiteboard(toolKey, toolName) {
      ...
      // Clone icon from card into whiteboard header
      var srcBtn  = document.querySelector('.note-btn[data-tool="' + toolKey + '"]');
      var srcIcon = srcBtn ? srcBtn.closest('.roadmap-item').querySelector('.item-icon') : null;
      var wrap    = document.getElementById('wbIconWrap');
      wrap.innerHTML = srcIcon ? srcIcon.innerHTML : '📋';

      // Load saved note text
      var savedText = Store.get('wb_note_' + toolKey) || '';
      var textarea  = document.getElementById('notesTextarea');
      textarea.value = savedText;

      var savedColor = Store.get('wb_color_' + toolKey);
      noteColor = savedColor || '#eef0f7';
      textarea.style.color = savedColor || '';

      // Sync color picker active state
      var colorBtns = document.querySelectorAll('.tb-color');
      for (var i = 0; i < colorBtns.length; i++) {
        colorBtns[i].classList.toggle('active', colorBtns[i].getAttribute('data-color') === noteColor);
      }

      // Render checklist, hide it initially
      ...
      var chkToggleBtn = document.getElementById('checklistToggleBtn');
      chkToggleBtn.style.borderColor = '';
      chkToggleBtn.style.color = '';
    ...
    function overlayClick(e) {
      if (e.target === document.getElementById('wbOverlay')) {
        closeWhiteboard();
      }
    }
    ...
    function saveNotes() {
      ...
      Store.set('wb_note_' + currentTool, text);
      Store.set('wb_color_' + currentTool, noteColor);

      // Save checklist
      try {
        Store.set('wb_checklists', JSON.stringify(checklistData));
      } catch(e) {}

      // Update the note button indicator on the card
      var btn = document.querySelector('.note-btn[data-tool="' + currentTool + '"]');
      var items = checklistData[currentTool];
      var hasContent = (text.trim().length > 0) || (items && items.length > 0);
      if (btn) {
        if (hasContent) {
      ...
      var saveBtn = document.getElementById('wbSaveBtn');
    ...
    // ─── CLEAR NOTES ───
    function clearNotes() {
      // Use a simple inline confirm — works in all Tomcat contexts
      var ok = window.confirm('Clear all notes and checklist for this tool?');
      if (!ok) { return; }
      document.getElementById('notesTextarea').value = '';
      if (currentTool) {
        checklistData[currentTool] = [];
        renderChecklist(currentTool);
        Store.remove('wb_note_' + currentTool);
        Store.remove('wb_color_' + currentTool);
        try { Store.set('wb_checklists', JSON.stringify(checklistData)); } catch(e) {}
        var btn = document.querySelector('.note-btn[data-tool="' + currentTool + '"]');
        if (btn) {
          btn.classList.remove('has-note');
          btn.textContent = '📝 Notes';
    ...
    function updateCharCount() {
      var len = document.getElementById('notesTextarea').value.length;
    ...
    function insertBullet() {
      ...
      var nl     = (before.length > 0 && before.charAt(before.length - 1) !== '\n') ? '\n' : '';
      ta.value   = before + nl + '• ' + after;
      ta.selectionStart = ta.selectionEnd = pos + nl.length + 2;
      ta.focus();
      updateCharCount();
    }
    ...
    function insertHeading() {
      ...
      var nl     = (before.length > 0 && before.charAt(before.length - 1) !== '\n') ? '\n' : '';
      ta.value   = before + nl + '## ' + after;
      ta.selectionStart = ta.selectionEnd = pos + nl.length + 3;
      ta.focus();
      updateCharCount();
    }
    ...
    function setColor(el) {
      noteColor = el.getAttribute('data-color');
      var colorBtns = document.querySelectorAll('.tb-color');
      for (var i = 0; i < colorBtns.length; i++) {
    ...
    function toggleChecklist() {
      ...
      var btn = document.getElementById('checklistToggleBtn');
    ...
    function renderChecklist(toolKey) {
      ...
      var items = checklistData[toolKey] || [];
      var html  = '';
      for (var i = 0; i < items.length; i++) {
        var item    = items[i];
        var checked = item.done ? ' checked' : '';
        html += '<div class="check-item">' +
          '<input type="checkbox" id="chk_' + i + '"' + checked + ' onchange="toggleCheck(' + i + ')">' +
    ...
    function addCheckItem() {
      ...
      checklistData[currentTool].push({ text: text, done: false });
      renderChecklist(currentTool);
      input.value = '';
      input.focus();
    }
    ...
    // ─── RESTORE NOTE BUTTON STATES ON PAGE LOAD ───
    (function restoreButtonStates() {
      var btns = document.querySelectorAll('.note-btn');
      for (var i = 0; i < btns.length; i++) {
        var key      = btns[i].getAttribute('data-tool');
        var noteText = Store.get('wb_note_' + key) || '';
        var items    = checklistData[key];
        var hasNote  = noteText.trim().length > 0;
        var hasCheck = items && items.length > 0;
        if (hasNote || hasCheck) {
          btns[i].classList.add('has-note');
          btns[i].textContent = '📝 Notes ●';
        }
      }
    })();
  </script>
